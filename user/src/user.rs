...
use proto::user::{
    user_service_server::UserService, GetUserProfileRequest, GetUserProfileResponse,
    LoginRequest, LoginResponse, RegisterRequest, RegisterResponse, UpdateUserProfileRequest,
    UpdateUserProfileResponse, User, VerifyRequest, VerifyResponse,
};
...
impl UserService for UserServiceImpl {
    ...
    async fn verify(
        ...
        let user = self.get_user_profile(Request::new(GetUserProfileRequest {
        ...
        if user_result.success && user_result.user.is_some() {
            Ok(Response::new(VerifyResponse {
                valid: true,
                user_id: user_result.user.as_ref().map(|u| u.user_id.clone()).unwrap_or_default(),
    ...
    async fn get_user_profile(
        ...
        match user_result {
            Some(user) => Ok(Response::new(GetUserProfileResponse {
                success: true,
